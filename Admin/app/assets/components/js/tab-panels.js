...
export default class TabPanelsElement extends LitElement {
    static properties = { 
    }
    static get styles() { };
     ...
     */
    render() {
	    return html`${ this.itemsToRender }`
    }
    ...
    showPanel(panelId){
        this.querySelectorAll(`app-panel[id]:not([id="${panelId}"]`).forEach((tabItem) => {
            tabItem.style.display = 'none'
        })
        this.querySelector(`app-panel[id="${panelId}"]`).style.display = 'block'
    }

    addPanel(panelContent, panelId){
        this.itemsToRender.push( html`<app-panel id="${panelId}" style="display: none">
                ${ panelContent }
            </app-panel>`)
        this.requestUpdate()
    }
...
// Register component
customElements.define('app-tab-panels', TabPanelsElement);